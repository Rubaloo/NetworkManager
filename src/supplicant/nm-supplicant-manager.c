...
static void _supp_iface_weak_cb (gpointer data,
...
NM_UTILS_LOOKUP_STR_DEFINE_STATIC (nm_supplicant_driver_to_string, NMSupplicantDriver,
                   ...
                   gboolean created_by_us)
{
	...
	supp_iface = nm_supplicant_interface_new (priv->dbus_connection,
	                                          priv->name_owner,
	                                          iface_path,
	                                          handle->ifindex,
	                                          handle->driver,
	...
	supp_iface->supp_iface_created_by_us = created_by_us;

	g_object_weak_ref (G_OBJECT (supp_iface),
	                   _supp_iface_weak_cb,
	                   self);
                            ...
                            gpointer user_data)
{
	...
	_supp_iface_remove_one (self, supp_iface, "D-Bus signal from supplicant");
}
...
static void
_supp_iface_remove_complete (NMSupplicantManager *self,
                             NMSupplicantInterface *supp_iface,
                             const char *reason)
{
	NMSupplicantManagerPrivate *priv = NM_SUPPLICANT_MANAGER_GET_PRIVATE (self);
	...
	name_owner = nm_supplicant_interface_get_dbus_name_owner (supp_iface);
	iface_path = nm_supplicant_interface_get_dbus_path (supp_iface);

	nm_assert (c_list_contains (&priv->supp_lst_head, &supp_iface->supp_lst));

	c_list_unlink (&supp_iface->supp_lst);
	if (!g_hash_table_remove (priv->supp_ifaces, iface_path))
		nm_assert_not_reached ();
	...
	_LOGD ("remove interface \"%s\" on %s (%s)%s",
	       ...
	       name_owner->str,
	       reason,
	       supp_iface->supp_iface_created_by_us ? " (remove in wpa_supplicant)" : "");

	if (supp_iface->supp_iface_created_by_us) {
		_dbus_call_remove_interface (priv->dbus_connection,
		                             name_owner->str,
		                             iface_path->str);
	}

	_nm_supplicant_interface_set_state_down (supp_iface);
}

static void
_supp_iface_weak_cb (gpointer data,
                     GObject *where_the_object_was)
{
	_supp_iface_remove_complete (data, NM_SUPPLICANT_INTERFACE (where_the_object_was), "object unreference");
}
                        ...
                        const char *reason)
{
	...
	g_object_weak_unref (G_OBJECT (supp_iface),
	                     _supp_iface_weak_cb,
	                     self);
	_supp_iface_remove_complete (self, supp_iface, reason);
}
                        ...
                        const char *reason)
{
	...
	while ((supp_iface = c_list_first_entry (&priv->supp_lst_head, NMSupplicantInterface, supp_lst)))
		_supp_iface_remove_one (self, supp_iface, reason);
}
                    ...
                    gboolean first_time)
{
	...
	if (first_time) {
		_LOGD ("wpa_supplicant name owner %s%s%s (%s)",
		       NM_PRINT_FMT_QUOTE_STRING (name_owner),
		       name_owner ? "running" : "stopped");
	} else {
		_LOGD ("wpa_supplicant name owner \"%s\" %s (%s)",
	...
	_supp_iface_remove_all (self, "name owner changed");
...
dispose (GObject *object)
{
	...
	_supp_iface_remove_all (self, "disposing");
